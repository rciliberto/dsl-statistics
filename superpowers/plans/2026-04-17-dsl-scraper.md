...
**Goal:** Build a Playwright-based scraper that collects Deadlock Death Slam tournament data from three sources (tournament site, statlocker.gg, Steam API) into SQLite, with a Jupyter notebook for analysis.

**Architecture:** Modular Python project with a CLI entry point (`scrape.py`) that orchestrates three scrapers (`scrapers/auth.py`, `scrapers/tournament.py`, `scrapers/statlocker.py`, `scrapers/steam.py`) and a database layer (`db.py`). Data flows: tournament site → divisions/teams/players → statlocker → stats/heroes/matches → Steam API → account metadata. Jupyter notebook queries SQLite directly.

**Tech Stack:** Python 3.11+, Playwright (Chromium), SQLite, pandas, matplotlib, seaborn, requests, python-dotenv

**Spec:** `docs/superpowers/specs/2026-04-17-dsl-scraper-design.md`
...
| File | Responsibility |
|------|---------------|
| `.gitignore` | Ignore .cookies.json, dsl.db, scrape.log, .env, .venv |
| `requirements.txt` | All Python dependencies |
| `db.py` | Schema creation, upsert helpers, query helpers |
| `scrapers/__init__.py` | Package init |
| `scrapers/auth.py` | Cookie load/save, interactive Discord login, session validation |
| `scrapers/tournament.py` | Scrape divisions, teams, rosters from tournament site |
| `scrapers/statlocker.py` | Scrape PP, rank, heroes, matches from statlocker via network interception |
| `scrapers/steam.py` | Steam Web API client for account age and games owned |
| `scrape.py` | CLI entry point with argparse, orchestrates all scrapers |
...
```bash
git init
git add .gitignore requirements.txt scrapers/__init__.py tests/__init__.py
...
def test_all_tables_exist(conn):
    ...
    assert "player_heroes" in tables
    assert "player_matches" in tables
...
def test_team_member_unique_constraint(conn):
    ...
    conn.execute("INSERT INTO teams (division_id, name, page_url) VALUES (1, 'Team A', 'http://a')")
    conn.execute("INSERT INTO players (display_name, steam_account_id) VALUES ('Alice', '123')")
    conn.execute(
...
- [ ] **Step 3: Implement `db.py` with schema creation**
...
RANK_NAMES = {
    0: "Obscurus", 1: "Initiate", 2: "Seeker", 3: "Alchemist",
    4: "Arcanist", 5: "Ritualist", 6: "Emissary", 7: "Archon",
...
- [ ] **Step 5: Write failing tests for upsert helpers**

Add to `tests/test_db.py`:
```python
from db import upsert_division, upsert_team, upsert_player, upsert_team_member
...
def test_upsert_player_updates_display_name(conn):
    ...
    upsert_player(conn, {
        "display_name": "Alice_New",
        "steam_account_id": "12345",
    })
    row = conn.execute("SELECT display_name FROM players WHERE steam_account_id = '12345'").fetchone()
...
def test_upsert_team_member_detects_leave(conn):
    ...
    from db import mark_departed_members
    # Second scrape: only Alice still on roster
    mark_departed_members(conn, team_id=1, current_player_ids=[1])
    row = conn.execute("SELECT left_at FROM team_members WHERE player_id = 2").fetchone()
...
- [ ] **Step 7: Implement upsert helpers in `db.py`**
...
def upsert_team(conn: sqlite3.Connection, data: dict) -> int:
    ...
    row = conn.execute("SELECT id FROM teams WHERE page_url = :page_url", data).fetchone()
    conn.commit()
    return row[0]
...
- [ ] **Step 9: Write failing tests for stats/match insert helpers**

Add to `tests/test_db.py`:
```python
from db import insert_player_stats, insert_player_heroes, insert_player_match, get_latest_stats_time

def test_insert_player_stats(conn):
    conn.execute("INSERT INTO players (display_name, steam_account_id) VALUES ('Alice', '1')")
    stats_id = insert_player_stats(conn, player_id=1, pp_score=1500.0, rank_number=9, rank_subrank=3)
    assert stats_id == 1
    row = conn.execute("SELECT pp_score, rank_number FROM player_stats WHERE id = 1").fetchone()
    assert row[0] == 1500.0
    assert row[1] == 9

def test_insert_player_match_dedup(conn):
    conn.execute("INSERT INTO players (display_name, steam_account_id) VALUES ('Alice', '1')")
    inserted = insert_player_match(conn, player_id=1, match_data={
        "match_id": "m1", "hero_name": "Haze", "pp_before": 1490.0,
        "pp_after": 1500.0, "pp_change": 10.0, "result": "win",
        "match_date": "2026-01-01",
    })
    assert inserted is True
    inserted2 = insert_player_match(conn, player_id=1, match_data={
        "match_id": "m1", "hero_name": "Haze", "pp_before": 1490.0,
        "pp_after": 1500.0, "pp_change": 10.0, "result": "win",
        "match_date": "2026-01-01",
    })
...
def test_insert_player_heroes(conn):
    ...
    rows = conn.execute("SELECT hero_name, matches_played FROM player_heroes WHERE stats_id = ?", (stats_id,)).fetchall()
    assert len(rows) == 2
...
def test_insert_player_heroes_dedup(conn):
    ...
    rows = conn.execute("SELECT COUNT(*) FROM player_heroes WHERE stats_id = ?", (stats_id,)).fetchone()
    assert rows[0] == 1

def test_upsert_player_preserves_existing_fields(conn):
    """Verify COALESCE logic: re-upserting without discord_name keeps existing value."""
    upsert_player(conn, {
        "display_name": "Alice",
        "discord_name": "alice#1234",
        "steam_account_id": "12345",
    ...
    row = conn.execute("SELECT display_name, discord_name FROM players WHERE steam_account_id = '12345'").fetchone()
    assert row[0] == "Alice_Updated"
    assert row[1] == "alice#1234"  # Should be preserved by COALESCE
```
...
- [ ] **Step 11: Implement stats/match insert helpers in `db.py`**
    ...
    for hero in heroes:
        conn.execute(
            """INSERT OR IGNORE INTO player_heroes
               ...
               VALUES (?, ?, ?, ?, ?)""",
            (stats_id, hero["hero_name"], hero.get("matches_played"),
             hero.get("win_rate"), hero.get("is_most_played", False)),
        )
    conn.commit()
    ...
    try:
        conn.execute(
            """INSERT INTO player_matches
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)""",
            (player_id, match_data["match_id"], match_data.get("hero_name"),
             match_data.get("pp_before"), match_data.get("pp_after"),
             match_data.get("pp_change"), match_data.get("result"),
...
git commit -m "feat: database layer with schema and upsert/insert helpers"
```

---

## Task 3: Cookie Management & Auth

**Files:**
- Create: `scrapers/auth.py`

This module manages Playwright session cookies for the Discord-authenticated tournament site. It cannot be unit tested in the traditional sense — it requires interactive browser login. We test it manually in Task 7.

- [ ] **Step 1: Implement `scrapers/auth.py`**
...
def is_logged_in(page: Page) -> bool:
    ...
    # If we get redirected to a login/accounts page, we're not authenticated
...
```bash
git add scrapers/auth.py
git commit -m "feat: Discord auth with cookie persistence and interactive login"
```

---

## Task 4: Tournament Site Scraper

**Files:**
- Create: `scrapers/tournament.py`

**This is scaffolding code.** The CSS selectors are best-guess placeholders. This code will NOT produce results until Task 8, where you run against the live site, inspect the actual HTML, and update the selectors to match.

- [ ] **Step 1: Implement `scrapers/tournament.py`**
...
def scrape_teams_list(page: Page, division_filter: str | None = None) -> list[dict]:
    ...
    teams = []
    # Strategy: look for division headers and team links beneath them.
    # The exact selectors will need adjustment after first manual inspection.
    # Common Django patterns: <h2>Division Name</h2> followed by <ul>/<table> of teams

    # This is a best-guess implementation. After first run, inspect the actual
    # HTML and update the selectors accordingly.
    content = page.content()
    logger.debug("Teams page loaded, URL: %s, content length: %d", page.url, len(content))

    # Try to find division sections and team links
    # Will need to be adapted to actual HTML structure
    divisions = page.query_selector_all("[class*='division'], [class*='Division'], h2, h3")
    logger.info("Found %d potential division headers", len(divisions))

    # Fallback: dump page structure for manual inspection
    if not divisions:
        logger.warning(
            "Could not find division headers. Save page HTML for inspection:\n"
            "page.content() in debugger or add --debug flag"
        )

    # TODO: This function's selectors need to be updated after first manual
    # inspection of the actual tournament site HTML structure.
    # For now, return raw data that can be parsed.
...
def scrape_team_page(page: Page, team_url: str, division: str) -> TeamData:
    ...
    Returns TeamData with all player information.
    """
    page.goto(team_url, wait_until="domcontentloaded")
    page.wait_for_load_state("networkidle")

    team_name_el = page.query_selector("h1, h2, [class*='team-name'], [class*='teamName']")
    ...
    # TODO: Selectors below need updating after first manual HTML inspection.
    # Look for player rows — common patterns:
    # - <table> rows with player info
    # - <div class="member"> or <li class="player"> blocks
    # - Links to steam profiles and statlocker
    ...
    for el in player_elements:
        try:
            ...
            logger.warning("Failed to parse player element in team '%s': %s", team_name, e)
...
def _parse_player_element(el) -> PlayerData | None:
    ...
    TODO: Update selectors after first manual HTML inspection.
    """
    ...
    # Try to determine role (core vs substitute)
...
```bash
git add scrapers/tournament.py
git commit -m "feat: tournament site scraper with team/player extraction"
```

**Note:** The CSS selectors in this file are best-guess placeholders. Task 7 (Integration Test) will involve running the scraper against the live site, inspecting the actual HTML, and updating these selectors to match.

---

## Task 5: Statlocker Scraper

**Files:**
- Create: `scrapers/statlocker.py`

Uses Playwright network interception to capture API responses from statlocker.gg's SPA.

- [ ] **Step 1: Implement `scrapers/statlocker.py`**
...
def scrape_player_stats(
    ...
    def capture_response(response):
        """Capture API responses from statlocker."""
        url = response.url
    ...
    try:
        ...
        logger.error("Failed to load statlocker profile %s: %s", steam_account_id, e)
        return data

    # Parse captured API responses
    for resp in api_responses:
        try:
            _parse_api_response(resp["url"], resp["data"], data, max_matches)
        except Exception as e:
...
def _parse_api_response(
    url: str, body: dict | list, data: StatlockerData, max_matches: int
) -> None:
    """Parse a captured API response and extract relevant data.

    The exact JSON structure depends on statlocker's API. This implementation
    covers common patterns — field names may need adjustment after inspecting
    actual API responses.

    Run with --debug flag to log raw API responses for inspection.
    """
    logger.debug("Parsing API response from: %s", url)
    logger.debug("Response body (truncated): %s", json.dumps(body, default=str)[:500])
    ...
    if isinstance(body, dict):
        ...
        for key in ("rank", "rankInfo", "rank_info"):
            if key in body and isinstance(body[key], dict):
                rank_data = body[key]
                data.rank_number = rank_data.get("tier", rank_data.get("rank", rank_data.get("number")))
                data.rank_subrank = rank_data.get("subrank", rank_data.get("sub_rank", rank_data.get("division")))
        ...
        for key in ("matches", "matchHistory", "match_history", "recentMatches", "games"):
            if key in body and isinstance(body[key], list):
                _parse_matches(body[key], data, max_matches)
...
def _parse_heroes(heroes_list: list, data: StatlockerData) -> None:
    ...
    for i, hero in enumerate(heroes_list):
        ...
        if not name:
            continue
        matches = hero.get("matches", hero.get("matches_played", hero.get("games", 0)))
        wins = hero.get("wins", 0)
        total = hero.get("matches", hero.get("games", matches))
        win_rate = hero.get("win_rate", hero.get("winRate", wins / total if total > 0 else 0))
        if isinstance(win_rate, (int, float)) and win_rate > 1:
            win_rate = win_rate / 100.0  # Convert percentage to decimal

        data.heroes.append(HeroStats(
            hero_name=name,
...
def _parse_matches(matches_list: list, data: StatlockerData, max_matches: int) -> None:
    """Parse match history from API response."""
    for match in matches_list[:max_matches]:
        if not isinstance(match, dict):
            continue
        match_id = str(match.get("match_id", match.get("matchId", match.get("id", ""))))
        if not match_id:
            continue
        ...
        if isinstance(result_raw, bool):
            ...
            result = "win" if result_raw.lower() in ("win", "won", "victory") else "loss"
        else:
            result = None

        pp_before = match.get("pp_before", match.get("ppBefore"))
        pp_after = match.get("pp_after", match.get("ppAfter"))
        pp_change = match.get("pp_change", match.get("ppChange", match.get("pp_delta")))

        if pp_before is not None and pp_after is not None and pp_change is None:
            pp_change = float(pp_after) - float(pp_before)

        date = match.get("date", match.get("match_date", match.get("played_at", match.get("timestamp"))))

        data.matches.append(MatchData(
            match_id=match_id,
            hero_name=hero,
...
@patch("scrapers.steam.requests.get")
...
- [ ] **Step 3: Implement `scrapers/steam.py`**
...
def fetch_player_summary(api_key: str, steam_account_id: str) -> dict:
    ...
    resp = requests.get(url, params={"key": api_key, "steamids": str(steam64)}, timeout=10)
...
def fetch_owned_games_count(api_key: str, steam_account_id: str) -> int | None:
    ...
    resp = requests.get(
        url,
        params={"key": api_key, "steamid": str(steam64), "include_played_free_games": True},
        timeout=10,
    )
    resp.raise_for_status()
...
def fetch_steam_info(api_key: str, steam_account_id: str) -> dict:
    ...
    Returns dict with: visible, account_created (datetime|None), games_owned (int|None).
    """
...
```bash
git add scrapers/steam.py tests/test_steam.py
git commit -m "feat: Steam API client for account age and games owned"
```
...
```python
#!/usr/bin/env python3
"""DSL Tournament Scraper — main entry point."""
...
from db import (
    ...
    upsert_division, upsert_team, upsert_player,
    upsert_team_member, mark_departed_members,
    insert_player_stats, insert_player_heroes, insert_player_match,
    get_latest_stats_time,
)
from scrapers.auth import get_authenticated_context
from scrapers.tournament import scrape_teams_list, scrape_team_page, extract_steam_account_id
from scrapers.statlocker import scrape_player_stats
from scrapers.steam import fetch_steam_info
...
def scrape_tournament(page, conn, division_filter=None, team_filter=None):
    ...
    logger = logging.getLogger("scrape.tournament")
    ...
    for team_info in teams_data:
        ...
        team_data = scrape_team_page(page, team_info["url"], team_info["division"])
        div_id = upsert_division(conn, team_data.division)
        team_id = upsert_team(conn, {
            "division_id": div_id,
            "name": team_data.name,
        ...
        for player in team_data.players:
            if not player.steam_account_id:
                logger.warning("Player '%s' has no steam account ID, skipping", player.display_name)
                continue
            ...
            upsert_team_member(conn, team_id, player_id, player.role, player.is_poc)
        ...
        logger.info("Team '%s' (%s): %d players", team_data.name, team_data.division, len(current_player_ids))

    return scraped_players

def scrape_statlocker_all(page, conn, players, force=False):
    """Scrape statlocker for all players."""
    logger = logging.getLogger("scrape.statlocker")
    stats_count = 0
    fail_count = 0
    ...
    for p in players:
        ...
        try:
            ...
            stats_id = insert_player_stats(
                conn, p["player_id"], data.pp_score, data.rank_number, data.rank_subrank
            )
            ...
            for match in data.matches:
                inserted = insert_player_match(conn, p["player_id"], {
            ...
            # Update first_game_at if available
            ...
            logger.info(
                ...
                p["display_name"], data.pp_score or 0, len(data.heroes), new_matches,
            )
        except Exception as e:
            fail_count += 1
            logger.error("Failed to scrape statlocker for '%s': %s", p["display_name"], e)

    return stats_count, fail_count

def scrape_steam_all(conn, players):
    """Fetch Steam account info for players missing it."""
    logger = logging.getLogger("scrape.steam")
    ...
    count = 0
    for p in players:
        # Skip if we already have Steam info
        row = conn.execute(
            "SELECT steam_profile_visible FROM players WHERE id = ?", (p["player_id"],)
        ).fetchone()
        ...
        try:
            ...
            conn.execute(
                """UPDATE players SET
                       ...
                       steam_games_owned = ?
                   WHERE id = ?""",
                (info["visible"], info["account_created"], info["games_owned"], p["player_id"]),
            )
            conn.commit()
            count += 1
            logger.info(
                "Steam info for '%s': visible=%s, games=%s",
                p["display_name"], info["visible"], info["games_owned"],
            )
        except Exception as e:
            logger.error("Failed to fetch Steam info for '%s': %s", p["display_name"], e)

    return count

def main():
    parser = argparse.ArgumentParser(description="DSL Tournament Scraper")
    parser.add_argument("--division", help="Scrape only this division")
    parser.add_argument("--team", help="Scrape only this team")
    parser.add_argument("--force", action="store_true", help="Ignore 24h cache")
    parser.add_argument("--debug", action="store_true", help="Enable debug logging")
    parser.add_argument("--skip-statlocker", action="store_true", help="Skip statlocker scrape")
    parser.add_argument("--skip-steam", action="store_true", help="Skip Steam API scrape")
    ...
    with sync_playwright() as p:
        # Step 1-2: Auth + Tournament scrape
        context = get_authenticated_context(p)
        page = context.new_page()

        logger.info("Scraping tournament site...")
        players = scrape_tournament(page, conn, args.division, args.team)
        logger.info("Found %d players from tournament site", len(players))

        page.close()
        context.close()

        # Step 3: Statlocker scrape (separate browser, no auth needed)
        if not args.skip_statlocker:
            logger.info("Scraping statlocker profiles...")
            browser = p.chromium.launch(headless=True)
            sl_context = browser.new_context()
            sl_page = sl_context.new_page()

            stats_count, fail_count = scrape_statlocker_all(sl_page, conn, players, force=args.force)

            sl_page.close()
            sl_context.close()
            browser.close()

            logger.info("Statlocker: %d scraped, %d failed", stats_count, fail_count)

    # Step 4: Steam API (no browser needed)
    if not args.skip_steam:
        logger.info("Fetching Steam account info...")
        steam_count = scrape_steam_all(conn, players)
...
```bash
git add scrape.py
git commit -m "feat: main CLI entry point orchestrating all scrapers"
```

---

## Task 8: Integration Test & Selector Refinement

**Files:**
- Modify: `scrapers/tournament.py` (update CSS selectors)
- Modify: `scrapers/statlocker.py` (update API field names)

This task requires running against the live sites. It cannot be automated.

- [ ] **Step 1: Test tournament site login**

Run: `python scrape.py --skip-statlocker --skip-steam --debug`
...
Update the CSS selectors in `scrapers/tournament.py`:
...
Run: `python scrape.py --team "YOUR_TEAM" --skip-statlocker --skip-steam --debug`
...
Run: `python scrape.py --team "YOUR_TEAM" --skip-steam --debug`

Check debug log for captured API responses. Update field names in `scrapers/statlocker.py` `_parse_api_response()` if needed.
...
Run: `python scrape.py --team "YOUR_TEAM" --skip-statlocker --debug`
...
- [ ] **Step 6: Full scrape test**

Run: `python scrape.py --division "Division 2" --debug`

Verify all data populated. Check scrape.log for errors.

- [ ] **Step 7: Commit selector fixes**

```bash
git add scrapers/tournament.py scrapers/statlocker.py
...
**Cell 3 — Section 1: Division Overview:**
```python
...
**Cell 4 — Section 2: Team Rankings:**
```python
# Average PP of core players per team
core_players = players_df[players_df["role"] == "core"]
team_rankings = core_players.groupby(["division", "team"])["pp_score"].agg(["mean", "count"]).round(1)
...
for div in sorted(players_df["division"].unique()):
    ...
    div_teams["Avg Core PP"].plot(kind="barh", ax=ax, color=sns.color_palette("viridis", len(div_teams)))
...
**Cell 5 — Section 3: Outlier Detection:**
```python
...
for div in sorted(players_df["division"].unique()):
    ...
    if len(outliers) > 0:
        ...
        for team, pp in outliers.items():
            direction = "ABOVE" if pp > mean else "BELOW"
            print(f"  {team}: {pp:.1f} ({direction}, {abs(pp - mean)/std:.1f}σ)")
        print()
```

**Cell 6 — Section 4: Player Distribution:**
```python
...
rank_counts.plot(kind="bar", ax=axes[1], color=sns.color_palette("coolwarm", len(rank_counts)))
...
**Cell 7 — Section 5: Scouting View:**
```python
def scout_player(player_name: str):
    """Display scouting info for a player."""
    player = players_df[players_df["display_name"].str.contains(player_name, case=False)]
    if player.empty:
    ...
    if not heroes.empty:
        ...
        for _, h in heroes.iterrows():
            print(f"  {h['hero_name']}: {h['matches_played']} games, {h['win_rate']*100:.0f}% WR")
    ...
    if not matches.empty and matches["pp_after"].notna().any():
        ...
        colors = ["green" if r == "win" else "red" for r in matches_sorted["result"]]
        ax.plot(range(len(matches_sorted)), matches_sorted["pp_after"], marker="o", color="steelblue")
        ax.scatter(range(len(matches_sorted)), matches_sorted["pp_after"], c=colors, zorder=5)
...
# Example: scout_player("YourName")
```

**Cell 8 — Section 6: Division Comparison:**
```python
fig, ax = plt.subplots(figsize=(10, 5))
div_order = sorted(players_df["division"].unique())
data = [players_df[players_df["division"] == d]["pp_score"].dropna() for d in div_order]
...
**Cell 9 — Section 7: PP Trends:**
```python
# PP trajectory from match history for top players in your division
my_division = "Division 2"  # Change as needed

div_players = players_df[players_df["division"] == my_division].nlargest(5, "pp_score")
...
for _, p in div_players.iterrows():
    ...
    if not matches.empty:
        ax.plot(range(len(matches)), matches["pp_after"], label=p["display_name"], marker=".")
...
**Cell 10 — Section 8: Alt Account Flags:**
```python
alt_flags = players_df[
    (players_df["pp_score"].notna()) &
    (
        (players_df["steam_profile_visible"] == False) |
        (players_df["steam_games_owned"].notna() & (players_df["steam_games_owned"] < 10)) |
        (players_df["steam_account_created"].notna() &
         (pd.to_datetime(players_df["steam_account_created"]) > pd.Timestamp("2024-06-01")))
    )
...
for _, p in alt_flags.iterrows():
    ...
    print(f"    PP: {p['pp_score']:.1f} | Rank: {p['rank_label']} | Flags: {', '.join(flags)}")
```
...
Run: `jupyter nbconvert --execute analysis.ipynb --to html`

(Requires data in `dsl.db` from Task 8)

- [ ] **Step 3: Commit**

```bash
git add analysis.ipynb
git commit -m "feat: analysis notebook with division stats, team rankings, scouting, and alt detection"
```
...
- [ ] **Step 3: Verify --force flag**

```bash
python scrape.py --division "Division 2" --force
```

Should re-scrape all statlocker profiles regardless of cache.

- [ ] **Step 4: Open and run analysis notebook**
